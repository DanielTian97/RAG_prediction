...
class ProbEstimator:
    def __init__(self, _ret='bm25', _task='nq_test', model_name="meta-llama/Meta-Llama-3-8B-Instruct", load_on_device=0):
        self.retriever = _ret
        self.task = _task
        ...
        if(torch.cuda.device_count() > 1):
            self.device = f'cuda:{load_on_device}'    
        
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModelForCausalLM.from_pretrained(model_name, device_map=self.device, dtype=torch.float16)
        self.tokenizer.pad_token = self.tokenizer.eos_token
        special_tokens_dict = {'additional_special_tokens': ['Q: ', 'A: ']}
        self.tokenizer.add_special_tokens(special_tokens_dict) # special token should be added at the end
        self.model.resize_token_embeddings(len(self.tokenizer))

        self.MAX_BATCH_FOR_PASSAGES = 8
    ...
    def individual_doc_probs(self, _k: int):
        output_path = f'./log_prob_temp_res/individual/{self.task}_{self.retriever}_{_k}.json'
        ...
        for qid in tqdm(self.res.qid.unique()):
            ...
            _i = 0
            while _i < min(_k, self.res[(self.res.qid==qid)].shape[0]):
    ...
    def individual_doc_probs_with_query(self, _k: int):
        output_path = f'./log_prob_temp_res/individual_with_query/{self.task}_{self.retriever}_{_k}.json'
        ...
        for qid in tqdm(self.res.qid.unique()):
            ...
            while _i < _k:
                ...
                # print(doc_texts_with_pre)

                avg_logProb_list = self.cal_part_logprob_in_batch_with_preamble(doc_texts_with_pre, self.device)
                torch.cuda.empty_cache()
                _res_per_q.update(dict(zip(range(_i, _i+len(doc_texts)), avg_logProb_list)))
                _i += _batch_size
