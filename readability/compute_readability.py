#!/usr/bin/env python3
"""Compute readability features for individual retrieved passages."""
...
def calculate_readability(text: str, docno: str, rank: int, qid, cache: dict):
    cached = cache.setdefault(docno, {})
    ...
    for name, method_name in METRICS.items():
        try:
            ...
            score = -1
            grade_level = []
        rows.append([qid, docno, rank, name, score, grade_level])
...
def main() -> None:
    ...
    output = args.output_dir / (
        f"individual_readability_{args.dataset}_{args.retriever}_top_{args.top_k}.csv"
    )
    cache_name = "nq_wiki" if "nq" in args.dataset else "msmarco"
    cache_path = args.output_dir / f"{cache_name}.pkl"
    ...
    for qid in tqdm(retrieval.qid.unique()):
        ...
        rows = []
        for text, docno, rank in zip(loaded.text, loaded.docno, loaded["rank"]):
            rows.extend(calculate_readability(text, str(docno), int(rank), qid, cache))
