...
This first-pass audit describes the current code, not a claim that the published results have been reproduced. Scientific source and stored results remain unchanged.
...
| Regression | `analysis/easy_analysis_readability-v2.ipynb` | Reads external labels/features and writes single/ensemble summaries |
| Tables and plots | `analysis/final_analysis_v3.ipynb` | Reads `ecir_res/single_output_v2.csv` and `union_output_v2.csv`; fresh-kernel issue below |
...
Paths are relative to the process working directory. Analysis notebooks expect `analysis/`; feature scripts generally expect their component directory. From these locations, `../../rag_utility` refers to a sibling directory. Its authoritative upstream repository revision is not recorded here.
...
NQ splits are `nq_dev` and `nq_test`; retriever aliases are `bm25`, `mt5`, and `e5`. Evaluation JSONs are indexed as `[qid]['0']['0']['F1']`. Generation JSONs supply `[qid]['0']['0']['probs']`, currently parsed from a string using `eval` in the regression notebook. Confirm the stored values' meaning from the upstream generator.
...
## Findings to resolve before scientific refactoring

1. **Notebook state:** `final_analysis_v3.ipynb` assigns the RPP subset to `_df`, then uses `rpp_df` in `df_dict`. That variable is undefined in a fresh kernel. Retained unchanged in this hygiene pass.
...
7. **Artifact identity:** several notebook versions, summary versions, and QPP backups coexist. The v2 summaries and v3 presentation notebook are candidates based on read/write links, not proven authoritative publication artifacts.
